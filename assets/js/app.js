...
(function ($) {
	...
	var Init = {
		...
		construct: function () {
			...
			this.helperSelecionarCartao();
			this.helperExcluirCartao();
		},
	   ...
	   */
		...
		*/
		...
		/**
		* Função que retorna um SVg com bandeira informada.
		* @method helperSVGBandeira
		*/
		helperSVGBandeira: function ($bandeira) {
			var $svg = "";
			if ($bandeira == "amex") {
				$svg = `<svg class="amex-logo" version="1.1" id="Layer_1" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns"
						xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
						viewBox="0 0 750 471" enable-background="new 0 0 752 471" xml:space="preserve">
						<g>
							<g>
								<path fill="#fff" d="M554.594,130.608l-14.521,35.039h29.121L554.594,130.608z M387.03,152.321
			...
			if ($bandeira == "maestro") {
				$svg = `<svg class="maestro-logo" viewBox="0 0 750 471" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
			...
			if ($bandeira == "mastercard") {
				$svg = `<svg class="mastercard-logo" viewBox="0 0 750 471" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
					    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
					        <g fill-rule="nonzero">
					            <rect x="0" y="0" rx="40"></rect>
					            <g transform="translate(133.000000, 48.000000)">
			...
			if ($bandeira == "visa") {
				$svg = `<svg class="visa-logo" viewBox="0 0 750 471" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
						    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
						        <g fill-rule="nonzero">
						            <rect x="0" y="0" rx="40"></rect>
			...
			if ($bandeira == "visaelectron") {
				$svg = `<svg class="visaelectron-logo" viewBox="0 0 256 114" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid">
						    <g>
			...
			if ($bandeira == "hipercard") {
				$svg = `<svg class="hipercard-logo" viewBox="0 0 750 471" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
						    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
						        <g fill-rule="nonzero">
			...
			if ($bandeira == "elo") {
				$svg = `<svg class="elo-logo" viewBox="0 0 750 471" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
						    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
						        <g fill-rule="nonzero">
					...
					<div class="content">
						...
						<form class="form form-login" method="POST" autocomplete="off">
							...
							<div class="form-group custom-material">
								<input type="email" name="login" class="form-control custom-field valida valida-email" value="">
								<label for="login">Login</label>
							</div>
							<div class="form-group custom-material">
								<input type="password" name="senha" class="form-control custom-field valida valida-senha" autocomplete="new-password" value="">
							...
							<div class="card" data-flag="false">
								<div class="front-card">
									<div class="content-card-cred">
										...
										<div class="card-logo">						
											`+
				Init.helperSVGBandeira("amex") +
				Init.helperSVGBandeira("maestro") +
				Init.helperSVGBandeira("mastercard") +
				Init.helperSVGBandeira("visa") +
				Init.helperSVGBandeira("visaelectron") +
				Init.helperSVGBandeira("hipercard") +
				Init.helperSVGBandeira("elo")
				+ `											
										</div>
									</div>
								</div>
		...
		*/
					...
					<div class="content">
						<ul class="lista-card-saved">
							<li class="selecionado">
								<div class="in">
									<div class="row">
										...
										<div class="col">
											<span class="text-num-cartao">***** **** **** 5236</span>
											<span class="data-vencimento">Vál. 02/24</span>
											<span class="status ativo">ATIVO</span>											
										</div>
									</div>
								</div>
								<div class="act"><a href="#" class="link-excluir">EXCLUIR</a></div>
							</li>
							<li>
								<div class="in">
									<div class="row">
										<div class="col-3">`+ Init.helperSVGBandeira("visa") + `</div>
										<div class="col">
											<span class="text-num-cartao">***** **** **** 5236</span>
											<span class="data-vencimento">Vál. 02/24</span>
							...
							<li>
								<div class="in">
									<div class="row">
										<div class="col-3">`+ Init.helperSVGBandeira("maestro") + `</div>
										<div class="col">
											<span class="text-num-cartao">***** **** **** 5236</span>
											<span class="data-vencimento">Vál. 02/24</span>
											<span class="status">ATIVO</span>
		...
		/**
		* Função que cria um helper para selecionar o cartão.
		* @method helperSelecionarCartao
		*/
		helperSelecionarCartao: function () {
			$("body").on("click", ".lista-card-saved .in", function () {
				if (!$(this).closest("li").hasClass("selecionado")) {
					$(this).closest(".lista-card-saved").find("li").removeClass("selecionado");
					$(this).closest("li").addClass("selecionado");
					$(this).closest(".lista-card-saved").find(".status").removeClass("ativo");
					$(this).closest("li.selecionado").find(".status").addClass("ativo");
					$(this).closest(".lista-card-saved").prepend($(this).closest("li"));
				}
			});
		},

		/**
		* Função que cria um helper para exluir o cartão.
		* @method helperExcluirCartao
		*/
		helperExcluirCartao: function () {
			$("body").on("click", ".lista-card-saved .link-excluir", function ($event) {
				...
				$(this).closest("li").addClass("has-alert");

				$("body").on("click", ".alert-confirm button", function () {
					...
					if ($confirm == 'yes') {
						...
						var $totalElements = $(this).closest(".lista-card-saved").children().length;

						if ($totalElements == 1) {
							$(this).closest(".lista-card-saved").html('<li class="text-no-card"><h3>Sem cartões cadastrados</h3></li>');
						}
					}
					if ($confirm == 'no') {
						$(this).closest("li").find('.alert-confirm').remove();
						$(this).closest("li").removeClass("has-alert");
					}
				});
			});
		},
		...
		*/
		verificaSeDataMenor: function ($data) {
			var $novaData = $data.split('/').reverse(),
				$dataInformada = new Date($novaData).valueOf(),
				$dataAtual = new Date().valueOf();
		...
		*/
		helperFinalizarPagamento: function () {
			$(".session-carrinho").on("click", ".btn-finalizar", function ($event) {
				...
				var $error = false, $totalError = 0;

				var $regexNome = /[a-z]\s[a-z]/gim,
					$regexCvv = /^[0-9]{3,4}$/;

				var $numberCard = $("#main-form-payment input[name='number-card']"),
					$dataValidCard = $("#main-form-payment input[name='expiry-date-card']"),
					$nameTitularCard = $("#main-form-payment input[name='name-card']"),
					$cvvCard = $("#main-form-payment input[name='cod-seg-card']"),
					$numberParcelas = $("#main-form-payment select[name='number-par']");

				if (!Init.helperValidaBandeiraCartao($numberCard.val())) {
					$error = true;
					$totalError++;
					$numberCard.addClass("error");
				}
				if (Init.verificaSeDataMenor($dataValidCard.val()) || $dataValidCard.val() === "") {
					$error = true;
					$totalError++;
					$dataValidCard.addClass("error");
				}
				if (!$regexNome.test($nameTitularCard.val())) {
					$error = true;
					$totalError++;
					$nameTitularCard.addClass("error");
				}
				if (!$regexCvv.test($cvvCard.val())) {
					$error = true;
					$totalError++;
					$cvvCard.addClass("error");
				}
				if ($numberParcelas.val() == "") {
					$error = true;
					$totalError++;
					$numberParcelas.closest(".custom-select").addClass("error");
				}

				//Verifica se exite erros
				if ($error === false || $totalError === 0) {
					$element.closest(".main").addClass("loading");
					$element.closest(".main").append(Init.helperSVGLoading());
					setTimeout(function () {
				...
				//Remove a classe de error
				$("body").on("keyup", "#main-form-payment input", function () {
